...
for (arrow_file in ArrowFiles){
  for (kval in k_values){
    for (LSI_m in LSI_methods){
      ...
      write.table(doubScores[[1]]$doubletEnrich,
                  sprintf("doublet_enrichment_scATAC_%s_knn_UMAP_kval_%g_LSImet_%g_UMAPneighbours_%g_UMAPmindist_%g_20210216.txt",
                          substr(arrow_file,1,12),kval, LSI_m, U_neigh, U_min), quote=FALSE, col.names=FALSE)
